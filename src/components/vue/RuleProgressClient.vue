<template>  <div>
    <div v-if="progress === null">
      ...
      <div class="text-xs text-center mt-1 text-gray-500 dark:text-gray-500">
    ...
    <div v-else>
      <div class="flex justify-between text-xs text-gray-600 dark:text-gray-400 mb-1">
        ...
        <div class="relative group">
          <span v-if="progress.mastered" class="text-emerald-500 dark:text-emerald-400 font-medium flex items-center">
            Maîtrisée ✓
            ...
            <div class="absolute hidden group-hover:block bg-gray-800 dark:bg-gray-950 text-white text-xs rounded py-1 px-2 right-0 bottom-full mb-1 w-48 z-10 border dark:border-gray-800">
              Une règle devient maîtrisée après 5 succès consécutifs.
            </div>
          </span>
          <span v-else-if="progress.consecutiveSuccesses > 0" class="text-amber-500 dark:text-amber-400 font-medium flex items-center">
      ...
      <div v-if="progress.successCount > 0 || progress.failureCount > 0" class="flex justify-between text-xs mt-1 dark:text-gray-400">
        <span>{{ progress.successCount }} succès</span>
        <span>{{ progress.failureCount }} échecs</span>
      </div>
      <div v-else class="text-xs text-center mt-1 text-gray-500 dark:text-gray-500">
        Pas encore pratiquée
      </div>
    </div>
  </div>
